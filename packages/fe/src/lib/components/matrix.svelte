<script lang="ts">
    ...
    function createAdjacencyMatrix(artist_data: Types.ArtistData[] | undefined) {
        ...
        for (let indexRow = 0; indexRow < matrix_size!; indexRow++) {  // creating each row
    ...
    function populateAdjacencyMatrix(influence_data: Types.ArtistInfluence[] | undefined, artist_data_with_index: Types.ArtistData[] | undefined){
        // console.log('influence_data: ', influence_data)
        // console.log('artist_data_with_index', artist_data_with_index)

        for(let [entry, ] of influence_data!.entries()){  // take each row in infuence_data
            // console.log('entry', entry)
            var row:number = 0
            var col = 0
            for(let [idx, ] of artist_data_with_index!.entries()){  // find index of the artist in artist_data 
    ...
    export const Initialize = (artist_data: Types.ArtistData[], influence_data: Types.ArtistInfluence[]) => {
        const sorted_artist_data = artist_data!
